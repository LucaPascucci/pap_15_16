...
bstFilter f (Node el l r)
  ...
  where
    --trova il nodo più a destra e lo aggancia a destra come sotto albero
    attach Nil t = t
    attach (Node el l r) t = Node el l (attach r t)

-- variant using lists
-- prende solo i valori che corrispondo al filtro (bstToList) e poi li inserisce in un nuovo albero (listToBst)
